            ...
            <div class="col-12 d-flex no-block align-items-center">
              ...
              <div class="ms-auto text-end">
                <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="/dbkopus">Dashboard</a></li>
            ...
            <div class="col-15">
              ...
              <div class="card shadow">
                <div class="card-body">
                  ...
                  {{-- Button in header --}}
                  {{-- <a href="{{ route('tabel_dokter.create') }}" class="card-title">
                    <button type="button" class="btn btn-primary">
                      Tambah Data +
                    </button>
                  </a> --}}
                  <a href="/cetak_pdf/tabel_dokter" class="card-title" target="_blank">
                    ...
                    <table id="zero_config" class="table table-striped table-bordered">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          {{-- <td>{{ $item->alamat }}</td> --}}
                          <td>{{ \Illuminate\Support\Str::limit($item->alamat , 50) }} <a href="/dbkopus/tabel_dokter/{{ $item->id_dokter }}/detail/" class="more-btn">  <strong> Read more » </strong></a></td>
                          <td>{{ $item->tempat }}</td>
                          <td>{{ $item->telpon }}</td>
                          {{-- <td>{{ $item->jadwal_kerja }}</td> --}}
                          <td>{{ \Illuminate\Support\Str::limit($item->jadwal_kerja , 50) }} <a href="/dbkopus/tabel_dokter/{{ $item->id_dokter }}/detail/" class="more-btn">  <strong> Read more » </strong></a></td>
                          <td>
                            @if ($item->id != 0)
                          ...
                          <td>
                            <div class="btn-group">
                              <a href="{{ route('tabel_dokter.edit',$item->id_dokter)}}" class="btn btn-cyan btn-sm text-white">Edit
                                <span class="fas fa-edit"></span></a>
                              {{-- <form action="{{ route('tabel_dokter.destroy',$item->id_dokter)}}" method="POST">
                              @csrf
                                  @method('DELETE')
                              ...
                              {{-- Note: Kopus bisa melihat seluruh data petugas keswan dan bisa mengeditnya. Namun kopus tidak bisa menambah/menghapus data --}}
                          </div>
                          </td>
                        </tr>
                        @endforeach
