            ...
            <div class="col-15">
                  ...
                  <div class="card shadow">
                    <div class="card-body">
                      ...
                      <a href="/cetak_pdf/dt_artikel" class="card-title" target="_blank">
                        ...
                        <table id="zero_config" class="table table-striped table-bordered">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>{!!\Illuminate\Support\Str::limit($item->isi , 50)!!} <a href="/dbstaf/dt_artikel/{{ $item->id_artikel }}/detail/" class="more-btn">  <strong> Read more » </strong></a></td>
                              <td><img src="/data/data_artikel/{{ $item->gambar }}" width="200"></td>
                              <td>{{ \Illuminate\Support\Str::limit($item->sumber , 20) }}</td>
                              <td>
                              <div class="btn-group">
                                  <a href="{{ route('dt_artikel.edit',$item->id_artikel)}}" class="btn btn-cyan btn-sm text-white">Edit
                                  ...
                                  <form action="{{ route('dt_artikel.batalkonfirmasi',$item->id_artikel)}}" method="POST">
                                    @csrf
                                        @method('PUT')
                                        <button type="submit" class="btn btn-warning btn-sm text-white" 
                                        onclick="return confirm('Apakah Anda yakin ingin mengarsipkan artikel {{ $item->judul }} ini?')">Arsipkan
                    ...
                    <table id="zero_config_2" class="table table-striped table-bordered">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                          <div class="btn-group">
                            <form action="{{ route('dt_artikel.konfirmasi',$item->id_artikel)}}" method="POST">
                              @csrf
                                  @method('PUT')
                                  <button type="submit" class="btn btn-success btn-sm text-white" 
                                  onclick="return confirm('Apakah Anda yakin ingin menampilkan artikel {{ $item->judul }} ini?')">Tampilkan
                                  <span class="fas fa-eye"></span></i></button>
                            </form>
                              <a href="{{ route('dt_artikel.edit',$item->id_artikel)}}" class="btn btn-cyan btn-sm text-white">Edit<span class="fas fa-edit"></span></a>
                              <form action="{{ route('dt_artikel.destroy',$item->id_artikel)}}" method="POST">
                              @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger btn-sm text-white" 
