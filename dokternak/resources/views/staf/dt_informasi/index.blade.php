            ...
            <div class="col-15">
                  ...
                  <div class="card shadow">
                    <div class="card-body">
                      ...
                      {{-- Button in header --}}
                      <a href="{{ route('dt_informasi.create') }}" class="card-title">
                        <button type="button" class="btn btn-primary">
                          Tambah Data +
                        </button>
                      </a>
                      <a href="/cetak_pdf/dt_informasi" class="card-title" target="_blank">
                        ...
                        <table id="zero_config" class="table table-striped table-bordered">
                          ...
                          <tbody>
                            ...
                            <tr>
                              <td>{{ $no++ }}</td>
                              <td>{{ $item->judul }}</td>
                              <td>{{ \Illuminate\Support\Str::limit($item->isi , 150) }} <a href="/dbstaf/dt_informasi/{{ $item->id_info }}/detail/" class="more-btn">  <strong> Read more » </strong></a></td>
                              <td>
                              <div class="btn-group">
                                  <a href="{{ route('dt_informasi.edit',$item->id_info)}}" class="btn btn-cyan btn-sm text-white">Edit
                                    <span class="fas fa-edit"></span></a>
                                  <form action="{{ route('dt_informasi.destroy',$item->id_info)}}" method="POST">
                                  @csrf
                                      @method('DELETE')
                                      <button type="submit" class="btn btn-danger btn-sm text-white" 
                                      onclick="return confirm('Apakah Anda yakin ingin menghapus informasi {{ $item->judul }} ')">Hapus
