            ...
            <div class="col-15">
              ...
              <div class="card shadow">
                <div class="card-body">
                  ...
                  {{-- Button in header --}}
                  <a href="{{ route('data_kopus.create') }}" class="card-title">
                    <button type="button" class="btn btn-primary">
                      Tambah Data +
                    </button>
                  </a>
                  <a href="/cetak_pdf/data_kopus" class="card-title" target="_blank">
                    ...
                    <table id="zero_config" class="table table-striped table-bordered">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{{ $item->telpon }}</td>
                          <td>{{ \Illuminate\Support\Str::limit($item->alamat , 50) }} <a href="/dashboard/data_kopus/{{ $item->id_kp }}/detail/" class="more-btn">  <strong> Read more » </strong></a></td>
                          <td>
                            @if ($item->id != 0)
                              <div class="text-success">Terdaftar</div> 
                              <a href="/dashboard/data_user_kopus" class="more-btn">  <strong> Lihat » </strong></a>
                            @else
                              <div class="text-danger">Belum Terdaftar</div> 
                              <a href="/dashboard/data_user_kopus/create" class="more-btn">  <strong> Daftarkan » </strong></a>
                            @endif 
                          </td>
                          {{-- <td>{{ $item->id != 0 ? 'Terdaftar' : 'Belum Terdaftar' }} }}</td> --}}
                          <td>
                            <div class="btn-group">
                              <a href="{{ route('data_kopus.edit',$item->id_kp)}}" class="btn btn-cyan btn-sm text-white">Edit
                                <span class="fas fa-edit"></span></a>
                              <form action="{{ route('data_kopus.destroy',$item->id_kp)}}" method="POST">
                              @csrf
                                  @method('DELETE')
